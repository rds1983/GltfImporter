	...
	public sealed class VertexContent
	{
		...
		private int SetupVertexDeclaration(VertexBufferContent result)
		{
			...
			foreach (var channel in Channels)
			{
				...
				else if (channel.ElementType == typeof(Color4D))
					format = VertexElementFormat.Color;
				else
					throw new Exception(string.Format("Unrecognized vertex content type: '{0}'", channel.ElementType));

				// Try to determine the vertex usage
				if (!VertexChannelNames.TryDecodeUsage(channel.Name, out usage))
					throw new Exception(string.Format("Unknown vertex element usage for channel '{0}'", channel.Name));
